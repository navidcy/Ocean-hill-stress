#See toml files for relevant versions of packages.
...
Nx, Nz = 750, 300		#Number of grid cells in x and z dimensions

architecture = GPU()		#Can be changed to CPU if needed

name = "mixedh100"		#This code is for a mixed flow test with a 100m hill

const H  = 1.5kilometers		#Ocean depth
const Lx = 500kilometers		#Domain horizontal length

#The following code is used to create a non-linear grid spacing with
#faces xnew_faces and znew_faces. This grid spacing creates a higher
...
const h0 = 100 # m		#Height of hill in metres
const width = 5000 #m		#Half width of hill in metres
bump(x, y) = - H + h0 * exp(-x^2 / 2width^2)		#Definition of the Gaussian hill
...
Δt = 7.5		#The time-step in seconds. Can be decreased if NANs are being produced.
...
@inline velocity_stabiliser(x,y,z,t,u)=(U_mean+U_tidal*cos(ω2 * t)-u)*ν2*(erf((x-(Lx/4))/(Lx/16))+erf((-x-(Lx/4))/(Lx/16))+2)+tidal_forcing(x, y, z, t)
...
stop_time = 750hours		#How long the simulation is run for
...
bottom_stress=Array{Float64}(undef,framesize,Nx)
for i in 1:framesize
    for j in 1:Nx
        bottom_stress[i,j]=(interior(bottom_pressure_t[i])[j])*(quadgk(bumpderiv,xnew_faces(j),xnew_faces(j+1),rtol=1e-10)[1])
    end
end
...
for i in 1:framesize
    stress_t[i]=0
    for j in 1:Nx
        stress_t[i]=stress_t[i]+bottom_stress[i,j]
    end
end
